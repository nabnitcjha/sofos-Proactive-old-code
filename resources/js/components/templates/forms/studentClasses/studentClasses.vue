...
<div class="student-wrapper">
        ...
        <h5 class="title text-left" v-if="todayTimetable.length>0"><span>Today</span> &nbsp; &nbsp;<span class="" style="font-size:16px; font-weight:500;">{{moment().format('ddd D MMM')}}</span></h5>
        <div class="row">
          <div class="col-sm-4 pointer-hand" v-for="(todaySession,index) in todayTimetable" :key="index"
            @click.stop="
              checkPermission(['update']) == true
                ? $root.changeRoute('/student-session-detail/' + todaySession.id)
                : fakeClick
            "
          >
            <div class="session-card mt-4">
              <div class="student-info">
                ...
                <img class="img-fluid student-profile-img"
                 :src="user.user_image == 0 ? adminImage:$root.getMedia(user.user_image)"     
                 >
              </div>
                <article class="student-content">
        ...
        <!-- upcoming class -->
        <div class="wrapper-content section-wrapper">
        <h5 class="title text-left mt-5" v-if="weeklyTimetable.length>0"><span>Upcoming Class(next 7 days)</span> &nbsp; &nbsp;<span class="" style="font-size:16px; font-weight:500;"></span></h5>
        <div class="row">
          <div class="col-sm-4 pointer-hand" v-for="(weeklySession,index) in weeklyTimetable" :key="index"
           @click.stop="
              checkPermission(['update']) == true
                ? $root.changeRoute('/student-session-detail/' + weeklySession.id)
                : fakeClick
            "
        ...
        <!-- history class -->
        <div class="wrapper-content section-wrapper">
        <h5 class="title text-left mt-5" v-if="previousTimeTable.length>0"><span>Previous Classes</span> &nbsp; &nbsp;<span class="" style="font-size:16px; font-weight:500;"></span></h5>
        <div class="row">
          <div class="col-sm-4 pointer-hand" v-for="(previousSession,index) in previousTimeTable" :key="index"
           @click.stop="
              checkPermission(['update']) == true
                ? $root.changeRoute('/student-session-detail/' + previousSession.id)
                : fakeClick
            "
  ...
  <!-- <div class="teacherclass-area col-12">
    ...
    <div
      class="student-class mb-5"
      v-for="(cls, index) in classes"
      :key="index"
      @click.stop="
        checkPermission(['update']) == true
          ? $root.changeRoute('/student-session-detail/' + cls.id)
          : fakeClick
      "
    >
      <img :src="SubIcon" alt="" />
...
export default {
  ...
  methods: {
    ...
    getStudentClassess() {  
      ...
      let userRoleName = this.user.roleNames[0];
      let url = '';
      
      if (userRoleName=='parent') {
        url = "/api/getParentClassess/" + this.user.parent.id;
      }else if (userRoleName=='student'){
        url = "/api/getStudentClassess/" + this.user.student.id;
      }else{
        url = "/api/getStudentClassess/" + this.$route.params.id;
      }
